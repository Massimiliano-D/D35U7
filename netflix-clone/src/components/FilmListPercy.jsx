...
class FilmListPercy extends React.Component {
  ...
  Percy = async () => {
    try {
      const response = await fetch("http://www.omdbapi.com/?i=tt3896198&apikey=a452cd0&s=percy%20jackson");
      const data = await response.json();
      const percyJacson = data.Search;
      console.log(data.Search);
      this.setState({ films: percyJacson });
    } catch (error) {
      console.log(error);
    }
  };
